...
export function formatTokens(count: number, locale: Locale): string {
  if (count >= 1000000) {
    return `${formatNumber(count / 1000000, locale, { maximumFractionDigits: 1 })}M`;
  } else if (count >= 1000) {
    return `${formatNumber(count / 1000, locale, { maximumFractionDigits: 1 })}K`;
  }
...
export function formatDuration(milliseconds: number, locale: Locale): string {
  ...
  if (seconds < 60) {
    return `${formatNumber(seconds, locale, { maximumFractionDigits: 2 })}s`;
  }

  const minutes = seconds / 60;
  if (minutes < 60) {
    return `${formatNumber(minutes, locale, { maximumFractionDigits: 1 })}m`;
  }
